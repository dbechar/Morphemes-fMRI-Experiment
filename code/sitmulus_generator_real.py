...
for par in range (num_par):
        ...
        for i_word in range(n_trials): # LOOP OVER TRIALS PER CONDITION
            ...
            # GENERATE A TARGET WORD AND VERIFY THAT IT DOES NOT ALREADY EXIST
             while True:
                ...
                if d_target_word['word'] not in target_words and d_control_word['word'] not in control_words and len(d_target_word['word']) <= 10:
    ...
    df_complete['type'] = 'real'
    
    # SAVE TRIALLIST IN CORRECT FOLDER
    path = f'../triallists/temp_lists/{language}_real/{str(par)}.csv'
